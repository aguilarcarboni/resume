...
type InternalCalendarEvent = Event & {
...
type ContextType = {
  ...
  locale: Locale;
  setEvents: (date: InternalCalendarEvent[]) => void;
  ...
  return (
    <div className="h-20 border-t last:border-b">
      {events
        ...
        .map((event) => {
          const eventStart = new Date(event.start);
          const eventEnd = event.ends ? new Date(event.ends) : addHours(eventStart, 1);
          ...
          >
            ...
            <span
              className={cn(
                ...
                isToday(date) &&
                  'bg-primary text-background rounded-md size-6'
              )}
            >
              {format(date, 'd')}
            </span>
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          const currentEvents = events.filter((event) =>
            isSameDay(new Date(event.start), _date)
          );

          return (
            <div
              className={cn(
                'ring-1 p-2 text-sm text-muted-foreground ring-muted overflow-auto',
            ...
            >
              <span
                className={cn(
                  ...
                  isToday(_date) && 'bg-primary text-background rounded-md'
                )}
              >
                {format(_date, 'd')}
              </span>
